    ...
    return fetch("/v1/status/public")
        ...
        .then(data => {
            if (data) {
                ...
                document.getElementById("evt-count-curr").innerHTML = evtCountCurrent;
                ...
                if (evtCount1hChange < 0) {
                    ...
                    document.getElementById("evt-count-1h").innerHTML = `${formatNumberShort(evtCount1hChange)}`;
                }
                ...
                if (evtCount1dChange < 0) {
                    ...
                    document.getElementById("evt-count-1d").innerHTML = `${formatNumberShort(evtCount1dChange)}`;
                }
                ...
                if (evtCount1wChange < 0) {
                    ...
                    document.getElementById("evt-count-1w").innerHTML = `${formatNumberShort(evtCount1wChange)}`;
                }

                const evtQueueCurrent = data.queueLength.current;
                document.getElementById("evt-queue-curr").innerHTML = evtQueueCurrent;
                const evtQueue1hChange = evtQueueCurrent - data.queueLength.past.hour;
                if (evtQueue1hChange < 0) {
                    document.getElementById("evt-queue-1h").innerHTML = `${formatNumberShort(evtQueue1hChange)}</span>`;
                } else {
                    document.getElementById("evt-queue-1h").innerHTML = `${formatNumberShort(evtQueue1hChange)}</span>`;
                }
                const evtQueue1dChange = evtQueueCurrent - data.queueLength.past.day;
                if (evtQueue1dChange < 0) {
                    document.getElementById("evt-queue-1d").innerHTML = `${formatNumberShort(evtQueue1dChange)}</span>`;
                } else {
                    document.getElementById("evt-queue-1d").innerHTML = `${formatNumberShort(evtQueue1dChange)}</span>`;
                }
                const evtQueue1wChange = evtQueueCurrent - data.queueLength.past.week;
                if (evtQueue1wChange < 0) {
                    document.getElementById("evt-queue-1w").innerHTML = `${formatNumberShort(evtQueue1wChange)}</span>`;
                } else {
                    document.getElementById("evt-queue-1w").innerHTML = `${formatNumberShort(evtQueue1wChange)}</span>`;
                }

                const subscriptionsCurrent = data.subscriptions.current;
                document.getElementById("subscriptions-curr").innerHTML = subscriptionsCurrent;
                ...
                document.getElementById("readers-curr").innerHTML = readersCurrent;
                ...
                document.getElementById("subscribers-curr").innerHTML = subscribersCurrent;
                ...
                document.getElementById("publishers-curr").innerHTML = publishersCurrent;
