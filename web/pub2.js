const templateSrc = (type, addr) => `
                <div class="hover:text-blue-500 hover:bg-gray-100 dark:hover:bg-gray-800 flex"
                     onclick="window.location.assign('pub-src-details2.html?type=${type}&addr=${encodeURIComponent(addr)}')">
    ...
    fetch(`/v1/src/${srcType}/list?limit=${pageLimit}&own=${own}&order=${order}&filter=${encodeURIComponent(filter)}`, {
        ...
        .then(data => {
            ...
            if (cursor === "") {
                ...
                btnPrev.onclick = () => {
                    window.location.assign(`pub2.html?cursor=${encodeURIComponent(data[0])}&order=DESC&filter=${encodeURIComponent(filter)}&srcType=${srcType}&own=${own}`)
                };
            }
            ...
            if (data != null && data.length === pageLimit) {
                ...
                btnNext.onclick = () => {
                    window.location.assign(`pub2.html?cursor=${encodeURIComponent(data[pageLimit - 1])}&order=ASC&filter=${encodeURIComponent(filter)}&srcType=${srcType}&own=${own}`)
                }
            } else {
            ...
            if (data == null || data.length === 0) {
                ...
                if (order === "ASC") {
                    ...
                    window.location.assign(`pub2.html?filter=${encodeURIComponent(filter)}&srcType=${srcType}&own=${own}`);
                }
            }
        })
        .catch(err => {
